...
// 这个头文件负责更高级的动态内存管理
// 包含一些基本函数 空间配置器 为初始化的储存空间管理 以及一个模板类 auto_ptr
...
namespace mystl
{
    ...
    pair<T *, ptrdiff_t> get_buffer_helper(ptrdiff_t len, T *)
    {
        if (len > static_cast<ptrdiff_t>(INT_MAX / sizeof(T)))
            len = INT_MAX / sizeof(T); // 防止申请内存超过最大数量
        while (len > 0)
        {
    ...
    template <class T>
    void
    release_temporary_buffer(T *ptr)
    {
        free(ptr);
    }
    ...
    // allocate_buffer 函数
    template <class ForwardIter, class T>
    inline void mystl::temporary_buffer<ForwardIter, T>::allocate_buffer()
    {
        original_len = len;
        if (len > static_cast<ptrdiff_t>(INT_MAX / sizeof(T)))
