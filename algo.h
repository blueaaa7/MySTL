...
{
    // all_of
    // 检查[first, last)内是否全部元素都满足一元操作，unary_pred为true的情况，满足则返回true
    ...
    // any_of
    // 检查[first, last)内是否存在某个元素满足一元操作unary_pred为true的情况，满足则返回true
    template <class InputIter, class UnaryPredicate>
    ...
    // none_of
    // 检查[first, last)内是否全部元素都不满足一元操作 unary_pred 为true的情况，满足则返回true
    template <class InputIter, class UnaryPredicate>
    ...
    // count_if
    // 对[first, last)区间内的每个元素都进行一元unary_pred操作，返回结果true的个数
    template <class InputIter, class UnaryPredicate>
    ...
    // find
    // 在[first, last)内找到等于value的元素 返回指向该元素的迭代器
    template <class InputIter, class T>
    InputIter
    find(InputIter first, InputIter lase, const T &value)
    {
    ...
    // find_if
    // 在[first ,last)内找到第一个令一元操作unary_pred为true的元素并返回指向该元素的迭代器
    template <class InputIter, class UnaryPredicate>
    InputIter
    ...
    // find_if_not
    // 在[first, last)内找到第一个令一元操作unary_pred为false的元素并返回迭代器
    template <class InputIter, class UnaryPredicate>
    InputIter
    ...
    // search
    // 在[first, last)中查找[first2, last2)的首次出现点
    ...
    ForwardIter1
    search(ForwardIter1 first1, ForwardIter1 last1, ForwardIter2 first2, ForwardIter2 last2, Compared comp)
    {
        auto d1 = mystl::distance(first1, last1);
        auto d2 = mystl::distance(first2, last2);
        if (d1 < d2)
    ...
    find_end_dispatch(ForwardIter1 first1, ForwardIter1 last1,
                      ...
                      mystl::forward_iterator_tag, mystl::forward_iterator_tag)
    {
        if (first2 == last2)
        {
            return last1;
    ...
    ForwardIter
    lbound_dispatch(ForwardIter first, ForwardIter last,
                    const T &value, mystl::forward_iterator_tag, Compared comp)
    {
        auto len = mystl::distance(first, last);
        auto half = len;
        ForwardIter middle;
    ...
    RandomIter
    lbound_dispatch(RandomIter first, RandomIter last,
                    const T &value, mystl::random_access_iterator_tag, Compared comp)
    {
        auto len = last - first;
        auto half = len;
        RandomIter middle;
    ...
    ubound_dispatch(ForwardIter first, ForwardIter last,
                    const T &value, mystl::forward_iterator_tag)
    {
        auto len = mystl::distance(first, last);
        auto half = len;
        ForwardIter middle;
    ...
    ubound_dispatch(RandomIter first, RandomIter last,
                    const T &value, mystl::random_access_iterator_tag)
    {
        auto len = last - first;
        auto half = len;
        RandomIter middle;
