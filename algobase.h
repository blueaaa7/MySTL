...
#undef min
    ...
    //unckecked_copy_backward_cat的bidirectional_iterator_tag版本
    ...
    //unchecked_copy_backward_cat的random_access_iterator_tag版本
    template<class RandomIter1,class BidirectionalIter2>
    BidirectionalIter2
    unchecked_copy_backward_cat(RandomIter1 first,RandomIter1 last,BidirectionalIter2 result,mystl::random_access_iterator_tag)
    {
    ...
    template <class BidirectionalIter1 ,class BidirectionalIter2>
    BidirectionalIter2
    copy_backward(BidirectionalIter1 first,BidirectionalIter2 last, BidirectionalIter2 result)
    {
        return unchecked_copy_backward(first,last,result);
    }

    //copy_if
    //把[first,last)内满足一元操作unary_pred的元素拷贝到以result为起始的位置上
    ...
    //bidirecitonal_iterator_tag版本
    template <class BidirectionalIter1, class BidirectionalIter2>
    BidirectionalIter2
    unchecked_move_cat(BidirectionalIter1 first,BidirectionalIter1 last,BidirectionalIter2 result,
    ...
    RandomIter2
    unchecked_move_backward_cat(RandomIter1 first,RandomIter2 last,RandomIter2 result, mystl::random_access_iterator_tag)
    {
        for(auto n = last - first; n>0; --n)
        {
            *--result = mystl::move(*--last);
    ...
    unchecked_move_backward(Tp * first, Up* result)
    {
    ...
    unchecked_fill_n(Tp* first, Size n,Up value)
    {
        if(n > 0)
        {
            std::meset(first, (unsigned char)value, (siez_t)(n));
        }
        return first + n;
    }
    ...
    {
        filll_n(first,last,value,iterator_category(first));
    }

    //lexicgraphical_compare
    //以字典序排序对两个序列进行比较，当在某个位置发现第一组不相等元素时，有下列几种情况：
    //1.如果第一序列的元素较小，返回true，否则返回false
    //2.如果到达last1 而尚未到达last2 返回true
    //3.如果到达last2 而尚未到达last1 返回false
    //4.如果同时到达last1 和 last2 返回false
    template <class InputIter1, class InputIter2>
